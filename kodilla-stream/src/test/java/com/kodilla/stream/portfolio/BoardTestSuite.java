...
public class BoardTestSuite {
    ...
    void testAddTaskListFindUsersTasks(){
        ...
        List<Task> tasks = project.getTaskLists().stream()
                ...
                .filter(t -> t.getAsignedUser().equals(user))
                .collect(toList());

        //Then
        assertEquals(2, tasks.size());
        assertEquals(user, tasks.get(0).getAsignedUser());
        assertEquals(user, tasks.get(1).getAsignedUser());
    }
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        //When
        var averageTimeTasks = new ArrayList<>();
        averageTimeTasks.add(new TaskList("In progress"));
        var averageTasks = project.getTaskLists().stream()
                .filter(averageTimeTasks::contains)
                .flatMap(t3 -> t3.getTasks().stream())
